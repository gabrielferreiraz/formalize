...
async function main() {
  ...
  const givagoCnpj = "43.778.093/0001-92";
  const givagoData = {
    name: "Givago",
    primaryColor: "#E8A045",
  ...
  const existingGivago = await prisma.artist.findFirst({ where: { cnpj: givagoCnpj } });
  const artist = existingGivago
    ? await prisma.artist.update({ where: { id: existingGivago.id }, data: givagoData })
    : await prisma.artist.create({ data: givagoData });
