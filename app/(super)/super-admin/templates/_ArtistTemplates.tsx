...
function ArtistAvatar({ artist, size = 36 }: { artist: Artist; size?: number }) {
  if (artist.logoUrl) {
    return (
      <img src={artist.logoUrl} alt={artist.name} width={size} height={size}
        className="rounded-xl object-cover shrink-0" style={{ width: size, height: size }} />
    );
  }
  return (
    <div className="rounded-xl flex items-center justify-center shrink-0 font-bold select-none"
      style={{ width: size, height: size, background: `${artist.primaryColor}20`, border: `1.5px solid ${artist.primaryColor}45`, color: artist.primaryColor, fontSize: size * 0.36 }}>
...
function NewTemplateModal({ artists, preselectedArtistId, onClose, onCreated }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const createRes = await fetch("/api/super/pdf-templates", {
        ...
        body: JSON.stringify({ artistId: form.artistId, type: form.type, name: form.name || `Template ${TYPE_LABEL[form.type]}`, pdfUrl: upJson.url, pageCount: upJson.pageCount }),
      });
  ...
  async function handleActivate() {
    if (!created) return;
    setActivating(true);
    await fetch(`/api/super/pdf-templates/${created.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: true }) });
    setActivating(false); onCreated(); onClose();
  }

  if (created) {
    return (
      <div className="fixed inset-0 z-[999] flex items-end sm:items-center justify-center bg-black/70"
        style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, width: "100%", height: "100dvh" }}
        onClick={(e) => e.target === e.currentTarget && onClose()}>
        <div className="bg-stage-800 border border-stage-600 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-sm overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            {[
              { label: "Posicionar campos", sub: "Abrir editor de canvas", onClick: () => router.push(`/super-admin/pdf-templates/${created.id}/editor`) },
              { label: "Testar PDF", sub: "Prévia com dados de exemplo", onClick: () => window.open(`/api/super/pdf-templates/${created.id}/test`, "_blank") },
              ...
              <button key={label} onClick={onClick}
                ...
                <div><p className="text-sm font-semibold text-gray-200">{label}</p><p className="text-xs text-gray-500">{sub}</p></div>
              </button>
            ))}
            ...
            <button onClick={onClose} className="w-full py-2.5 rounded-xl border border-stage-600 text-gray-500 hover:text-gray-300 text-sm transition-colors">Fechar</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-end sm:items-center justify-center bg-black/70"
      style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, width: "100%", height: "100dvh" }}
      onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="bg-stage-800 border border-stage-600 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-stage-600/60">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl text-gray-500 hover:text-gray-200 hover:bg-stage-700 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["contrato", "orcamento"] as const).map((t) => (
                <button key={t} type="button" onClick={() => set("type", t)}
                  className={`py-2.5 rounded-xl border text-sm font-semibold transition-all ${form.type === t ? "border-gold-500/60 bg-gold-500/10 text-gold-400" : "border-stage-500 text-gray-400 hover:border-stage-400 hover:text-gray-200"}`}>
                  {TYPE_LABEL[t]}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="text" className="input-field" placeholder={`Template ${TYPE_LABEL[form.type]} 2025`} value={form.name} onChange={(e) => set("name", e.target.value)} />
          </div>
          <div>
            ...
            {file ? (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-green-500/30 bg-green-500/5">
                ...
                <button type="button" onClick={() => { setFile(null); if (fileRef.current) fileRef.current.value = ""; }} className="shrink-0 text-gray-500 hover:text-red-400 transition-colors">
              ...
              <button type="button" onClick={() => fileRef.current?.click()}
                ...
                className={`w-full rounded-xl p-6 text-center transition-all ${dragging ? "border-2 border-gold-500/70 bg-gold-500/8" : "border-2 border-dashed border-stage-500 hover:border-gold-500/50 hover:bg-stage-700/40"}`}>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl border border-stage-500 text-gray-400 hover:text-gray-200 text-sm font-medium transition-colors">Cancelar</button>
            <button type="submit" disabled={loading} className="flex-1 py-3 rounded-xl bg-gold-500 hover:bg-gold-400 text-stage-900 text-sm font-bold transition-colors disabled:opacity-50">{loading ? "Enviando…" : "Criar template"}</button>
          </div>
        </form>
      </div>
    </div>
...
function TemplateCard({ mapping, onToggleActive, onDelete, deleting }: {
  mapping: Mapping; onToggleActive: () => void; onDelete: () => void; deleting: boolean;
}) {
  const dot = TYPE_DOT[mapping.type];
  return (
    <div className={`flex flex-col rounded-2xl border bg-stage-800 transition-all overflow-hidden ${mapping.isActive ? "border-gold-500/30" : "border-stage-600 hover:border-stage-500"}`}>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          {mapping.isActive && (
            <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-gold-500/12 border border-gold-500/30 text-gold-400 font-bold tracking-wide">
              <span className="w-1.5 h-1.5 rounded-full bg-gold-400 animate-pulse" /> Ativo
            </span>
          )}
        </div>
        <p className="text-sm font-bold text-gray-100 truncate mb-1">{mapping.name}</p>
        <div className="flex items-center gap-3 text-xs text-gray-600">
          <span>{mapping.pageCount} {mapping.pageCount === 1 ? "página" : "páginas"}</span>
          <span className="text-stage-500">·</span>
          <span>{(mapping.fields as unknown[]).length} campos</span>
        </div>
      </div>
      <div className="flex items-center border-t border-stage-700/80 divide-x divide-stage-700/80">
        ...
        <button onClick={onToggleActive}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium transition-colors hover:bg-stage-700/50 ${mapping.isActive ? "text-yellow-500" : "text-green-500"}`}>
...
export default function ArtistTemplatesSection() {
  ...
  useEffect(() => {
    fetch("/api/super/artists").then((r) => r.json()).then((data: Artist[]) => setArtists(Array.isArray(data) ? data : []));
  }, []);

  function loadMappings(artistId: string) {
    setLoadingMappings(true);
    fetch(`/api/super/pdf-templates?artistId=${artistId}`).then((r) => r.json()).then((data) => setMappings(Array.isArray(data) ? data : [])).finally(() => setLoadingMappings(false));
  }

  function selectArtist(a: Artist) { setSelectedArtist(a); loadMappings(a.id); }

  async function toggleActive(m: Mapping) {
    await fetch(`/api/super/pdf-templates/${m.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isActive: !m.isActive }) });
  ...
  return (
    <>
      {showNew && (
        <NewTemplateModal artists={artists} preselectedArtistId={selectedArtist?.id ?? ""}
          onClose={() => setShowNew(false)} onCreated={() => selectedArtist && loadMappings(selectedArtist.id)} />
      )}
      <div className="flex gap-0 overflow-hidden" style={{ height: "100%" }}>
        {/* Sidebar */}
        <div className="w-60 shrink-0 border-r border-stage-700/80 flex flex-col bg-stage-900/60">
          <div className="px-3 py-3 border-b border-stage-700/80">
            <div className="relative">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar artista…"
          ...
          <div className="flex-1 overflow-y-auto py-1.5 px-2 space-y-0.5">
            {filteredArtists.length === 0 && (
              <p className="text-xs text-gray-700 text-center py-8">{artists.length === 0 ? "Carregando…" : "Nenhum resultado"}</p>
            )}
            {filteredArtists.map((a) => {
              const isSelected = selectedArtist?.id === a.id;
              return (
                <button key={a.id} onClick={() => selectArtist(a)}
                  className={`w-full flex items-center gap-2.5 px-2.5 py-2 rounded-xl text-left transition-all ${isSelected ? "bg-stage-700/80 border border-stage-500" : "border border-transparent hover:bg-stage-800 hover:border-stage-700/60"}`}>
                  <ArtistAvatar artist={a} size={30} />
                  <p className={`text-xs font-semibold truncate flex-1 ${isSelected ? "text-gray-100" : "text-gray-400"}`}>{a.name}</p>
                  {isSelected && (
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#f5c200" strokeWidth="2.5" strokeLinecap="round" className="shrink-0"><polyline points="9 18 15 12 9 6" /></svg>
                  )}
                </button>
              );
            })}
              ...
              <div className="flex-1 overflow-y-auto p-5">
                {loadingMappings && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {[1,2,3].map((i) => <div key={i} className="rounded-2xl border border-stage-700 h-36 animate-pulse bg-stage-800/60" />)}
                  </div>
                )}
                {!loadingMappings && mappings.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full gap-4 text-center py-16">
                    ...
                    <p className="text-xs text-gray-600">Crie o primeiro template para {selectedArtist.name}</p>
                    <button onClick={() => setShowNew(true)} className="px-4 py-2 rounded-xl bg-gold-500/10 border border-gold-500/30 text-gold-400 text-sm font-semibold hover:bg-gold-500/15 transition-colors">+ Criar template</button>
                  </div>
                )}
                {!loadingMappings && mappings.length > 0 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                    {mappings.map((m) => (
                      <TemplateCard key={m.id} mapping={m} onToggleActive={() => toggleActive(m)} onDelete={() => handleDelete(m)} deleting={deletingId === m.id} />
                    ))}
                  </div>
                )}
              </div>
